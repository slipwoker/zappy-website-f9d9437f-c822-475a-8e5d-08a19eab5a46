        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            inputs.forEach(function(input) {
                ...
                if (input.type === 'email' && input.value.trim() && !validateEmail(input.value.trim())) {
                    showError(input, 'يرجى إدخال بريد إلكتروني صالح');
                    isValid = false;
                }
                
                if (input.hasAttribute('minlength') && input.value.trim().length < parseInt(input.getAttribute('minlength'))) {
                    showError(input, 'يجب أن يكون ' + input.getAttribute('minlength') + ' أحرف على الأقل');
                    isValid = false;
                }
            });
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'f9d9437f-c822-475a-8e5d-08a19eab5a46',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'f9d9437f-c822-475a-8e5d-08a19eab5a46';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'f9d9437f-c822-475a-8e5d-08a19eab5a46';
